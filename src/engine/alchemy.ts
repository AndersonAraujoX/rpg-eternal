...
export const brewPotion = (p: Potion, resources: Resources, costMultiplier: number = 1.0): { success: boolean, cost: Partial<Resources>, error?: string } => {
    // Check costs
    for (const c of p.cost) {
        const actualCost = Math.floor(c.amount * costMultiplier);
        if ((resources[c.type] || 0) < actualCost) {
    ...
    p.cost.forEach(c => cost[c.type as keyof Resources] = Math.floor(c.amount * costMultiplier));
