...
describe('Unification Systems: Relics, Resonance, Void Infusion & Stats Integration', () => {
    ...
    it('should buy, equip and unequip relics', async () => {
    ...
    it('should upgrade elemental resonance', async () => {
        ...
        act(() => {
            ...
            result.current.setElementalResonance(prev => ({...prev, fire: 1}));
            result.current.setElementalEssences(prev => ({...prev, fire: 10}));
        });
        await new Promise(r => setTimeout(r, 50));
    ...
    it('should infuse items with void', async () => {
        const { result } = renderHook(() => useGame());
