    ...
    return (
        <div className="absolute inset-0 z-40 flex items-center justify-center bg-black bg-opacity-95">
            <div className="bg-indigo-950 border-4 border-indigo-500 w-full max-w-lg p-6 rounded-lg shadow-2xl relative text-center">
                ...
                <h2 className="text-indigo-200 text-2xl font-bold mb-4 flex items-center justify-center gap-2"><Castle /> TORRE DA ETERNIDADE</h2>

                <div className="mb-6">
                    <div className="text-4xl font-bold text-white mb-2">ANDAR {tower.floor}</div>
                    <div className="text-indigo-400 text-sm">Andar Máximo: {tower.maxFloor}</div>
                </div>

                <div className="bg-black bg-opacity-50 p-4 rounded mb-6 border border-indigo-800">
                    <p className="text-gray-300 text-sm mb-2">Regras da Torre:</p>
                    <ul className="text-left text-xs text-indigo-300 list-disc pl-5 space-y-1">
                        <li>Os inimigos não respawnam.</li>
                        <li>Se seu grupo for aniquilado, você é ejetado.</li>
                        <li>A dificuldade aumenta exponencialmente (x1.5 HP por andar).</li>
                        <li>Recompensas: Conhecimento Ancestral (Em breve).</li>
                    </ul>
                </div>
                ...
                <div className="bg-red-950 bg-opacity-40 p-3 rounded mb-6 border border-red-700">
                    <div className="text-red-400 font-bold mb-1 uppercase tracking-wider flex items-center justify-center gap-2 text-sm">
                        ☠️ Mutador Diário: {getDailyMutator().name}
                    </div>
                ...
                >
                    {tower.active ? <><Skull /> RECUAR</> : <><Castle /> ENTRAR NA TORRE</>}
                </button>

                <div className="mt-6 border-t border-indigo-800 pt-4">
                    <div className="text-yellow-400 font-bold mb-2 text-xl">✨ Luz Estelar: {starlight || 0} ✨</div>
                    <button
                    ...
                    <p className="text-xs text-cyan-400 mt-1">Reseta a Torre para ganhar Luz Estelar com base no Andar Máximo.</p>
                </div>
            </div>
        </div>
    );
