    ...
    return (
        <div className="absolute inset-0 z-40 flex items-center justify-center bg-black bg-opacity-95">
            <div className="bg-gray-900 border-4 border-purple-500 w-full max-w-lg p-4 rounded-lg shadow-2xl relative">
                ...
                <h2 className="text-center text-purple-400 text-xl font-bold mb-4 flex items-center justify-center gap-2"><Ghost /> LOJA DE TALENTOS DE ALMA</h2>

                <div className="flex justify-between items-center mb-4 bg-gray-800 p-2 rounded">
                    <div className="text-white">Almas: <span className="text-purple-400 font-bold">{souls}</span></div>
                ...
                {souls > 1000 && (
                    <div className="mb-4 bg-yellow-900 p-2 rounded border border-yellow-500 text-center animate-pulse">
                        <button onClick={actions.triggerAscension} className="text-yellow-300 font-bold w-full uppercase">Ascender (Ganhar Divindade)</button>
                    </div>
                )}
                ...
                <div className="grid grid-cols-1 gap-2 max-h-[40vh] overflow-y-auto mb-4">
                    {talents.map(t => (
                        <div key={t.id} className="bg-gray-800 p-3 rounded flex justify-between items-center border border-gray-700">
                            <div>
                                <div className="text-yellow-300 font-bold text-sm">{t.name} <span className="text-xs text-gray-500">(Nvl {t.level}/{t.maxLevel})</span></div>
                            ...
                            >
                                {t.level >= t.maxLevel ? 'MÁX' : `Melhorar x${buyAmount} (~${t.cost * buyAmount})`}
                            </button>
                        </div>
                    ))}
                </div>
                ...
                {boss.level > 10 && (
                    <div className="mt-2 text-center border-t border-gray-600 pt-2">
                        <div className="text-gray-400 text-xs mb-1">Travado? Resete por Almas.</div>
                        <button onClick={actions.triggerRebirth} className="btn-retro bg-red-900 text-red-200 border border-red-500 px-6 py-2 rounded hover:bg-red-800 w-full">RENASCER ( +{Math.floor(boss.level / 5)} Almas )</button>
                    </div>
                )}
            </div>
        </div>
