...
export const VoidModal: React.FC<VoidModalProps> = ({ isOpen, onClose, voidMatter, actions, voidAscensions, voidActive, voidTimer }) => {
    ...
    return (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black bg-opacity-95">
            <div className="bg-purple-950 border-4 border-purple-600 w-full max-w-lg p-6 rounded-lg shadow-2xl relative text-purple-100">
                ...
                <h2 className="text-purple-400 text-3xl font-bold mb-4 flex items-center justify-center gap-2 font-mono"><Ghost /> DIMENSÃO DO VAZIO</h2>

                <div className="flex flex-col gap-4 justify-center mb-6">
                    <button
                        onClick={() => { actions.enterVoid(); onClose(); }}
                        disabled={voidActive}
                        className={`px-6 py-3 font-bold rounded shadow-lg flex items-center gap-2 justify-center transition-all ${voidActive ? 'bg-gray-800 text-gray-400 cursor-not-allowed border-gray-600' : 'bg-purple-600 hover:bg-purple-500 text-white animate-pulse'}`}
                    >
                        <Skull /> {voidActive ? `NA DIMENSÃO (${voidTimer}s)` : 'ENTRAR NA DIMENSÃO DO VAZIO (30s)'}
                    </button>

                    <div className="border-t border-purple-800 pt-4 mt-2">
                        <h3 className="text-center font-bold text-red-400 mb-2">DESAFIO FINAL</h3>
                        <button
                        ...
                        >
                            ...
                            {voidAscensions >= 5 ? "INVOCAR O NÚCLEO DO VAZIO" : `BLOQUEADO (Ascensões: ${voidAscensions}/5)`}
                        </button>
                    </div>
                </div>

                <h3 className="text-xl font-bold text-purple-300 mb-2 border-b border-purple-800 pb-1">Dons das Trevas</h3>
                <div className="grid grid-cols-1 gap-2">
                    <div className="flex justify-between items-center bg-purple-900 p-2 rounded border border-purple-800">
                        <div className="flex items-center gap-2">
                            <Zap className="text-yellow-400" size={16} />
                            <span>Carga do Vazio (Ultimate Instantâneo)</span>
                        </div>
                        <button
